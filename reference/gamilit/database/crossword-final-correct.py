#!/usr/bin/env python3
"""
Crucigrama correcto basado en la imagen
Con todas las intersecciones válidas
"""
...
# Crear grid
grid = [[' ' for _ in range(17)] for _ in range(10)]
...
# Mostrar grid
print("=" * 75)
print("CRUCIGRAMA CORREGIDO - Sin conflictos")
print("=" * 75)
print("     ", end="")
for c in range(17):
    print(f"{c:2}", end=" ")
    ...
    for j, word2 in enumerate(words):
        ...
        for k, letter in enumerate(word1['answer']):
            if word1['direction'] == 'horizontal':
                positions1.append((word1['startRow'], word1['startCol'] + k, letter, k))
            else:
                positions1.append((word1['startRow'] + k, word1['startCol'], letter, k))

        for k, letter in enumerate(word2['answer']):
            if word2['direction'] == 'horizontal':
                positions2.append((word2['startRow'], word2['startCol'] + k, letter, k))
            else:
                positions2.append((word2['startRow'] + k, word2['startCol'], letter, k))
...
print(f"Total de conflictos: {len(conflicts)}")

if len(conflicts) == 0 and len(intersections) >= 5:
    print("\n✅ EXCELENTE: Crucigrama funcional sin conflictos")
